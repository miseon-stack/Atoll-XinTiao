...
struct SettingsView: View {
    ...
    private var settingsSearchIndex: [SettingsSearchEntry] {
        [
            ...
            // Shortcuts
            SettingsSearchEntry(tab: .shortcuts, title: "Quick Launcher", keywords: ["launcher", "panel", "keyboard", "shortcut", "快捷启动", "快捷键", "启动器"], highlightID: SettingsTab.shortcuts.highlightID(for: "Quick Launcher")),
...
struct Shortcuts: View {
    ...
    @ObservedObject private var shortcutLauncher = AtollShortcutLauncherService.shared
        ...
        Form {
            Section {
                LabeledContent("Status") {
                    HStack(spacing: 7) {
                        Circle()
                            .fill(shortcutLauncher.state == .running ? Color.green : Color.orange)
                            .frame(width: 7, height: 7)
                        Text(shortcutLauncher.state.title)
                            .foregroundStyle(.secondary)
                    }
                }

                LabeledContent("Panel shortcut") {
                    Text(shortcutLauncher.panelHotkeyDisplayName)
                        .font(.system(.body, design: .monospaced))
                }

                if case .unavailable(let failure) = shortcutLauncher.state {
                    Text(failure.message)
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }

                Button(shortcutLauncher.state == .running ? "Open Quick Launcher" : "Start Quick Launcher") {
                    shortcutLauncher.presentPanel()
                }
                .disabled(shortcutLauncher.state.isTransitioning)
            } header: {
                Text("Quick Launcher")
            } footer: {
                Text("Open the panel from the notch menu, the menu bar, or the global panel shortcut. Configure bindings, direct shortcuts, website icons, import, and export inside the panel.")
                    .multilineTextAlignment(.trailing)
                    .foregroundStyle(.secondary)
                    .font(.caption)
            }
            .settingsHighlight(id: highlightID("Quick Launcher"))
