...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private var isWaitingForApplicationTermination = false
...
//    private var windowSizeUpdateWorkItem: DispatchWorkItem?
    ...
    func applicationShouldTerminate(_ sender: NSApplication) -> NSApplication.TerminateReply {
        ...
        guard !isWaitingForApplicationTermination else { return .terminateLater }
        isWaitingForApplicationTermination = true
        ...
        Task { @MainActor [weak self, weak sender] in
            ...
            await self?.shortcutLauncherService.stop()
            self?.isWaitingForApplicationTermination = false
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Register Atoll's existing shortcuts first. ShortcutLauncher then
        // reports any user-customized collision without stealing the host key.
        if !AppRuntimeEnvironment.isTesting {
            Task { @MainActor [shortcutLauncherService] in
                _ = await shortcutLauncherService.start()
            }
        }
